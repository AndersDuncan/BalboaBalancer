...
static float kp = 2; //kp 0.9
static float kd = 0.55; //kd 0.07
static float ki = 12.6;
int32_t eInt , ePrev = 0;
static int setpoint = 9500;
int16_t controlPID(){
  int32_t e = setpoint - angle;
  int32_t eDot = (e - ePrev) / 0.01;
  eInt = eInt + e * 0.01;
  int32_t u = kp * e + ki * eInt + kd * eDot; 
  int16_t out = (u/1000) * -1; // dividing by 1000 because millidegrees, -1 for motor reverse
  ePrev = e;
  return out;
}
...
void balance()
{
  ...
  //Serial.print(angle);
  //Serial.print("\nRAO:");
  //Serial.print(risingAngleOffset);
  //Serial.print("\nPID:");
  //Serial.print(controlPID());
  /*motorSpeed += (
